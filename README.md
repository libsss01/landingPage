# Landing Page LLM

Ce depot contient une landing page statique qui presente trois grands assistants IA et LLM : ChatGPT, Claude et Gemini. Le projet est construit en HTML et CSS pur, avec une structure simple en plusieurs sections.

## But du Projet

Projet a titre d'apprentissage du developpement front-End avec les bases du developpement Wen (HTML,CSS). La page sert de mini vitrine pour expliquer rapidement l'interet des outils d'intelligence artificielle generative et comparer, de maniere introductive, quelques modeles populaires.

## Fonctionnalites

- Header avec logo et liens de navigation.
- Hero section avec texte principal, bouton et image.
- Section d'information sur ChatGPT, Claude et Gemini.
- Bloc citation sur l'evolution de la programmation avec l'IA.
- Call-to-action final.
- Footer avec copyright.

## Stack technique

- HTML5
- CSS3

## Structure du projet

```txt
landingPage/
|-- index.html
|-- index.css
`-- assets/
    |-- ChatGPT-Logo.png
    |-- claude.png
    |-- Gemini.jpeg
    `-- Gemini_Generated_Image_mmv4hmmv4hmmv4hm.png
```

## Lancement local

Aucune dependance n'est necessaire.

1. Cloner le depot :

```sh
git clone https://github.com/libsss01/landingPage.git
```

2. Ouvrir `index.html` dans un navigateur.


## Contenu de la page

- `first-section` : header et hero.
- `second-section` : presentation des LLM.
- `third-section` : citation.
- `fourth-section` : appel a l'action.
- `footer` : copyright.

## Notes

Le contenu de la page est editorial et peut etre mis a jour avec le temps, surtout pour les textes qui parlent des "meilleurs LLM" ou des tendances IA d'une annee precise.

## Pistes d'amelioration

- Responsive design
- Rendre les liens du header fonctionnels avec des ancres.
- Ajouter des liens externes vers les pages officielles des outils presentes.
- Ajouter un bouton CTA menant vers une ressource concrete.
- Ajouter des balises meta SEO et Open Graph.

## Auteur

Mame Libasse Laye Sylla
